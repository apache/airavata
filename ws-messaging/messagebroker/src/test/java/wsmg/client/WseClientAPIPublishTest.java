...
public class WseClientAPIPublishTest extends TestCase {
    ...
    // basic pubilsh test
    @Test
    public final void testPublishPlainText() {

        try {
            String brokerURL = configs.getProperty(ConfigKeys.BROKER_EVENTING_SERVICE_EPR);
            String topic = "MYSimpleTOPIC";

            // should throw an exception
            EndpointReference brokerEPR = new EndpointReference(brokerURL);
            WseClientAPI clientApi = new WseClientAPI(brokerEPR);

            clientApi.publishPlainText("Plain text to publish with out topic");
            clientApi.publishPlainText(brokerURL, topic, "Plain text to publish with topic");
