...
// ClusterCredentialController serves /api/v1/cluster-credentials.
type ClusterCredentialController struct{ svc *ClusterCredentialService }

// NewClusterCredentialController returns a handler delegating to svc.
func NewClusterCredentialController(svc *ClusterCredentialService) *ClusterCredentialController {
	return &ClusterCredentialController{svc: svc}
}

// Register mounts the cluster credential routes.
//
// The literal /me pattern takes precedence over /{id}, so "me" is never mistaken for
// a binding id.
func (h *ClusterCredentialController) Register(mux *http.ServeMux) {
	mux.HandleFunc("GET /api/v1/cluster-credentials", h.list)
	mux.HandleFunc("GET /api/v1/cluster-credentials/me", h.listMine)
	mux.HandleFunc("POST /api/v1/cluster-credentials", h.create)
	mux.HandleFunc("GET /api/v1/cluster-credentials/{id}", h.get)
	mux.HandleFunc("PUT /api/v1/cluster-credentials/{id}", h.update)
	mux.HandleFunc("DELETE /api/v1/cluster-credentials/{id}", h.delete)
}

func (h *ClusterCredentialController) list(w http.ResponseWriter, r *http.Request) {
	found, err := h.svc.List(r.Context(), r.URL.Query().Get("clusterId"))
	if err != nil {
		httpx.WriteError(w, r, err)
		return
	}
...
func (h *ClusterCredentialController) listMine(w http.ResponseWriter, r *http.Request) {
	found, err := h.svc.ListMine(r.Context(), r.URL.Query().Get("clusterId"))
	if err != nil {
		httpx.WriteError(w, r, err)
		return
	}
...
func (h *ClusterCredentialController) get(w http.ResponseWriter, r *http.Request) {
...
func (h *ClusterCredentialController) create(w http.ResponseWriter, r *http.Request) {
	var req dto.ClusterCredentialRequest
	if err := httpx.Bind(r, &req); err != nil {
		httpx.WriteError(w, r, err)
		return
	}
...
func (h *ClusterCredentialController) update(w http.ResponseWriter, r *http.Request) {
	var req dto.ClusterCredentialRequest
	if err := httpx.Bind(r, &req); err != nil {
		httpx.WriteError(w, r, err)
		return
...
func (h *ClusterCredentialController) delete(w http.ResponseWriter, r *http.Request) {
