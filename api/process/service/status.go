...
// StatusService records and reads Process status history.
//
...
func (s *StatusService) Record(ctx context.Context, processID string, status model.ProcessStatusType, log *string) (*dto.StatusResponse, error) {
...
func (s *StatusService) RecordTx(ctx context.Context, tx *gorm.DB, processID string, status model.ProcessStatusType, log *string) (*model.ProcessStatus, error) {
	if _, err := s.processes.WithTx(tx).FindByID(ctx, processID); err != nil {
		return nil, notFoundAs(err, "Process not found: %s", processID)
	}

	created := &model.ProcessStatus{
		ProcessID: &processID,
		Status:    &status,
		Log:       log,
