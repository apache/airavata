...
public class GetExperimentOutputs {
    ...
    public static List<DataObjectType> getExperimentOutputs(String expId){
    	AiravataUtils.setExecutionAsClient();
    	List<DataObjectType> outputs = null;
        try {
    	...
    	try {
			outputs = client.getExperimentOutputs(expId);
    ...
    public static void main(String[] args) {
    	...
    	readConfigFile();
    	String expId = args[0];
    	List<DataObjectType> outputs = getExperimentOutputs(expId);  
    	for(int i=0; i<outputs.size(); i++){
    		System.out.println(outputs.get(i).getValue());
    	}
    }
