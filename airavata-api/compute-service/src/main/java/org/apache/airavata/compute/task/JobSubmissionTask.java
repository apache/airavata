...
public abstract class JobSubmissionTask extends AiravataTask {
    ...
    @SuppressWarnings("WeakerAccess")
    public void saveJobModel(JobModel jobModel) throws Exception {
    ...
    public void saveAndPublishJobStatus(JobModel jobModel) throws Exception {
        try {
            ...
            JobStatus jobStatus;
            if (jobModel.getJobStatusesList() != null
                    && !jobModel.getJobStatusesList().isEmpty()) {
                jobStatus = jobModel.getJobStatusesList().get(0);
            } else {
                logger.error("Job statuses can not be empty");
                return;
            }

            // Ensure timeOfStateChange is set
