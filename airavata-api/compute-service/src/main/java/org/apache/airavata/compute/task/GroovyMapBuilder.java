...
public class GroovyMapBuilder {
    ...
    public GroovyMapData build() throws Exception {
        ...
        List<String> inputValues =
                getProcessInputValues(taskContext.getProcessModel().getProcessInputsList(), true);
        inputValues.addAll(getProcessOutputValues(taskContext.getProcessModel().getProcessOutputsList(), true));
        mapData.setInputs(inputValues);

        List<String> inputFiles =
                getProcessInputFiles(taskContext.getProcessModel().getProcessInputsList(), false);
        mapData.setInputFiles(inputFiles);

        List<String> inputValuesAll =
                getProcessInputValues(taskContext.getProcessModel().getProcessInputsList(), false);
        inputValuesAll.addAll(
                getProcessOutputValues(taskContext.getProcessModel().getProcessOutputsList(), false));
        ...
        List<String> hostAliases = taskContext.getComputeResourceDescription().getHostAliasesList();
        ...
        List<SetEnvPaths> exportCommands = appDepDescription.getSetEnvironmentList();
        ...
        List<CommandObject> moduleCmds = appDepDescription.getModuleLoadCmdsList();
        if (moduleCmds != null) {
        ...
        List<CommandObject> preJobCommands = appDepDescription.getPreJobCommandsList();
        ...
        List<CommandObject> postJobCommands = appDepDescription.getPostJobCommandsList();
        ...
        if (parallelism != null) {
            if (parallelism != ApplicationParallelismType.SERIAL) {
                Map<Integer, String> parallelismPrefix =
                        taskContext.getResourceJobManager().getParallelismPrefixMap();
                if (parallelismPrefix != null) {
                    String parallelismCommand = parallelismPrefix.get(parallelism.getNumber());
    ...
    private static void setMailAddresses(TaskContext taskContext, GroovyMapData groovyMap) throws Exception {
        ...
        if (ServerSettings.getSetting(ServerSettings.JOB_NOTIFICATION_ENABLE).equalsIgnoreCase("true")) {
            ...
            if (processModel.getEnableEmailNotification()) {
                List<String> emailList = processModel.getEmailAddressesList();
                if (emailList == null) {
