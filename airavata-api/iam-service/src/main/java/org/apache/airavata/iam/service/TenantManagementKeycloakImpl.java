...
import org.apache.airavata.model.user.proto.Status;
...
public class TenantManagementKeycloakImpl implements TenantManagementInterface {
    ...
    private static Client getResteasyClient() {
        ...
        try {
            if (ServerSettings.isTLSEnabled()) {
                // TLS client trust configuration would go here
                // Currently disabled in airavata-server.properties
            }
    ...
    public Gateway addTenant(PasswordCredential isSuperAdminPasswordCreds, Gateway gatewayDetails)
        ...
        try {
            ...
            throw new IamAdminServicesException(
                    "Error getting Iam server Url from property file, reason: " + ex.getMessage());
        } catch (Exception ex) {
            logger.error("Error creating Realm in Keycloak Server, reason: " + ex.getMessage(), ex);
            throw new IamAdminServicesException("Error creating Realm in Keycloak Server, reason: " + ex.getMessage());
        } finally {
            if (client != null) {
                client.close();
            }
    ...
    public boolean createTenantAdminAccount(
        ...
        try {
            ...
            logger.error("Error getting values from property file, reason: " + ex.getMessage(), ex);
            throw new IamAdminServicesException("Error getting values from property file, reason " + ex.getMessage());
        } catch (Exception ex) {
            logger.error("Error creating Realm Admin Account in keycloak server, reason: " + ex.getMessage(), ex);
            throw new IamAdminServicesException(
                    "Error creating Realm Admin Account in keycloak server, reason: " + ex.getMessage());
        } finally {
            if (client != null) {
                client.close();
            }
    ...
    public Gateway configureClient(PasswordCredential isSuperAdminPasswordCreds, Gateway gatewayDetails)
        ...
        try {
            ...
            if (gatewayDetails.getGatewayUrl() != null) {
                ...
                throw new IamAdminServicesException(
                        "Gateway Url field in GatewayProfile cannot be empty, Realm Client creation failed");
            }
            ...
            if (httpResponse.getStatus() == 201) {
                ...
                CredentialRepresentation clientSecret = client.realms()
                        ...
                        .get(ClientUUID)
                        .getSecret();
                return gatewayDetails.toBuilder()
                        .setOauthClientId(pgaClient.getClientId())
                        .setOauthClientSecret(clientSecret.getValue())
                        .build();
            } else {
    ...
    public boolean resetUserPassword(String accessToken, String tenantId, String username, String newPassword)
        ...
        try {
            ...
            throw new IamAdminServicesException(
                    "Error resetting user password in keycloak server, reason: " + ex.getMessage());
        } finally {
            if (client != null) {
                client.close();
            }
    ...
    public List<UserProfile> findUser(String accessToken, String tenantId, String email, String userName)
        ...
        try {
            ...
            if (!retrieveUserList.isEmpty()) {
                ...
                for (UserRepresentation user : retrieveUserList) {
                    UserProfile profile = UserProfile.newBuilder()
                            ...
                            .addAllEmails(Arrays.asList(user.getEmail()))
                            .build();
            ...
            throw new IamAdminServicesException("Error finding user in keycloak server, reason: " + ex.getMessage());
        } finally {
            if (client != null) {
                client.close();
            }
    ...
    public void updateUserProfile(String accessToken, String tenantId, String username, UserProfile userDetails)
        ...
        try {
            ...
            if (userRepresentation != null) {
                ...
                userRepresentation.setEmail(userDetails.getEmailsList().get(0));
                UserResource userResource = client.realm(tenantId).users().get(userRepresentation.getId());
            ...
            throw new IamAdminServicesException(
                    "Error updating user profile in keycloak server, reason: " + ex.getMessage());
        } finally {
            if (client != null) {
                client.close();
            }
    ...
    public boolean deleteUser(String accessToken, String tenantId, String username) throws IamAdminServicesException {
        ...
        try {
            ...
            throw new IamAdminServicesException("Error deleting user in keycloak server, reason: " + ex.getMessage());
        } finally {
            if (client != null) {
                client.close();
            }
    ...
    private UserProfile convertUserRepresentationToUserProfile(UserRepresentation userRepresentation, String tenantId) {
        ...
        return UserProfile.newBuilder()
                .setAiravataInternalUserId(userRepresentation.getUsername() + "@" + tenantId)
                .setGatewayId(tenantId)
                .setUserId(userRepresentation.getUsername())
                .setFirstName(userRepresentation.getFirstName())
                .setLastName(userRepresentation.getLastName())
                .addAllEmails(Arrays.asList(userRepresentation.getEmail()))
                .setCreationTime(userRepresentation.getCreatedTimestamp())
                .setLastAccessTime(0)
                .setValidUntil(0)
                .setState(state)
                .build();
