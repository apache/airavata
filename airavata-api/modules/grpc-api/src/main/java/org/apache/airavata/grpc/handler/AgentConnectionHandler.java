...
* under the License.
*/
package org.apache.airavata.grpc.handler;
...
import org.apache.airavata.agent.AsyncCommandListResponse;
import org.apache.airavata.agent.AsyncCommandTerminateRequest;
import org.apache.airavata.agent.AsyncCommandTerminateResponse;
...
import org.apache.airavata.agent.TunnelTerminationRequest;
import org.apache.airavata.agent.TunnelTerminationResponse;
...
public class AgentConnectionHandler extends AgentCommunicationServiceGrpc.AgentCommunicationServiceImplBase
        implements AgentConnectionService {
    ...
    private final String tunnelServerHost;
    private final int tunnelServerPort;
    private final String tunnelServerApiUrl;
    private final String tunnelServerToken;

    public AgentConnectionHandler(
            @Value("${airavata.services.agent.tunnelserver.host}") String tunnelServerHost,
            @Value("${airavata.services.agent.tunnelserver.port}") int tunnelServerPort,
            @Value("${airavata.services.agent.tunnelserver.url}") String tunnelServerApiUrl,
            @Value("${airavata.services.agent.tunnelserver.token}") String tunnelServerToken) {
        this.tunnelServerHost = tunnelServerHost;
        this.tunnelServerPort = tunnelServerPort;
        this.tunnelServerApiUrl = tunnelServerApiUrl;
        this.tunnelServerToken = tunnelServerToken;
    }
    ...
    public AgentAsyncCommandListResponse getAsyncCommandListResponse(String executionId) {
        ...
        if (ASYNC_COMMAND_LIST_RESPONSE_CACHE.containsKey(executionId)) {
            ...
            agentCommandListResponse.setCommands(commandsList.stream()
                    .map(c -> {
                        var cmd = new org.apache.airavata.agent.model.AsyncCommand();
