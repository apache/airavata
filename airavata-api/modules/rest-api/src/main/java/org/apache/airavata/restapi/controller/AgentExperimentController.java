...
import org.apache.airavata.agent.service.AgentManagementService;
...
@RestController
@RequestMapping("/api/v1/agent/experiments")
@Tag(name = "Agent Experiments")
public class AgentExperimentController {

    private final AgentManagementService agentManagementHandler;

    public AgentExperimentController(AgentManagementService agentManagementHandler) {
        this.agentManagementHandler = agentManagementHandler;
    }

    @GetMapping(value = "/{expId}", produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<Experiment> getExperiment(@PathVariable("expId") String expId) {
    ...
    public ResponseEntity<AgentLaunchResponse> createAndLaunchExperiment(
            @Validated @RequestBody AgentLaunchRequest request) {
        AgentLaunchResponse agentResponse = agentManagementHandler.createAndLaunchExperiment(request);
        return ResponseEntity.ok(agentResponse);
    }

    @PostMapping(value = "/launch-optimize", consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<AgentLaunchResponse> createAndLaunchOptimizedExperiment(
            @Validated @RequestBody List<AgentLaunchRequest> request) throws Exception {
    ...
    @PostMapping(value = "/{expId}/terminate", produces = MediaType.APPLICATION_JSON_VALUE)
