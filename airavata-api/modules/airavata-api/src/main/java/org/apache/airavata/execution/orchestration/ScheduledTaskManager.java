...
public class ScheduledTaskManager {
    ...
    public ScheduledTaskManager(
            ...
            @org.springframework.lang.Nullable ReScheduler reScheduler) {
     ...
     */
    private static final ProcessState[] SCANNABLE_STATES = {ProcessState.QUEUED, ProcessState.REQUEUED};
    ...
    public void scanProcesses() {
        ...
        try {
            for (ProcessState state : SCANNABLE_STATES) {
                var processes = processService.getProcessListInState(state);
                for (var process : processes) {
                    reScheduler.reschedule(process, state);
                }
                logger.debug("Scanned {} processes in state {}", processes.size(), state);
            }
        } catch (Exception e) {
