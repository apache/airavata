...
/**
 * Default implementation of {@link CredentialEntityService} with encryption/decryption support.
 */
@Service
@Transactional
public class DefaultCredentialEntityService implements CredentialEntityService {

    private static final Logger logger = LoggerFactory.getLogger(DefaultCredentialEntityService.class);
    ...
    public DefaultCredentialEntityService(
    ...
    private Credential entityToCredential(CredentialEntity entity) throws CredentialStoreException {
        ...
        credential.setCreatedAt(
                entity.getCreatedAt() != null ? entity.getCreatedAt().toEpochMilli() : 0L);
        credential.setDescription(entity.getDescription());
        return credential;
    }

    /**
     * Determine the type discriminator enum for a credential.
     */
    private CredentialType credentialType(Credential credential) {
        return switch (credential) {
            case SSHCredential _ -> CredentialType.SSH;
            case PasswordCredential _ -> CredentialType.PASSWORD;
            case CertificateCredential _ -> CredentialType.CERTIFICATE;
        };
    }

    /**
     * Determine the concrete class for a type discriminator enum.
     */
    private Class<? extends Credential> credentialClass(CredentialType type) throws CredentialStoreException {
        return switch (type) {
     ...
     */
    private Credential deserializeCredential(CredentialType type, byte[] data) throws CredentialStoreException {
        try {
            if (shouldEncrypt()) {
