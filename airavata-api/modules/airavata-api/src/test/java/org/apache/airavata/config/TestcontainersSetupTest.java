...
public class TestcontainersSetupTest {
    ...
    public void testKeyTablesExist() throws Exception {
        try (Connection conn = dataSource.getConnection()) {
            ...
            assertTrue(tableExists(conn, "RESOURCE") || tableExists(conn, "resource"), "RESOURCE table should exist");
            assertTrue(
                    tableExists(conn, "EXPERIMENT") || tableExists(conn, "experiment"),
                    "EXPERIMENT table should exist");
            assertTrue(
                    tableExists(conn, "EVENT") || tableExists(conn, "event"),
                    "EVENT table (unified status/events) should exist");
        }
    }
