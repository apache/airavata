...
import org.apache.airavata.model.appcatalog.parser.proto.*;
import org.apache.airavata.model.application.io.proto.OutputDataObjectType;
import org.apache.airavata.model.experiment.proto.ExperimentModel;
import org.apache.airavata.model.process.proto.ProcessModel;
import org.apache.airavata.orchestration.task.*;
import org.apache.airavata.orchestration.task.ParsingTaskInput;
import org.apache.airavata.orchestration.task.ParsingTaskInputs;
import org.apache.airavata.orchestration.task.ParsingTaskOutput;
import org.apache.airavata.orchestration.task.ParsingTaskOutputs;
import org.apache.airavata.orchestration.task.ProcessCompletionMessage;
import org.apache.airavata.orchestration.task.ProcessCompletionMessageDeserializer;
...
public class ParserWorkflowManager implements IServer {
    ...
    private boolean process(ProcessCompletionMessage completionMessage) {
        ...
        try {
            ...
            for (ParsingTemplate template : parsingTemplates) {
                for (ParserConnector connector : template.getParserConnectionsList()) {
            ...
            for (ParsingTemplate template : parsingTemplates) {
                ...
                ParserInput parserInput = registryHandler.getParserInput(
                        template.getInitialInputsList().get(0).getTargetInputId(), template.getGatewayId());
                ...
                DataParsingTask parentParserTask = createParentTask(
                        parentParser, completionMessage, template.getInitialInputsList(), registryHandler);
    ...
    private DataParsingTask createParentTask(
        ...
        for (ParsingTemplateInput templateInput : templateInputs) {
            ...
            Optional<ParserInput> parserInputOp = parserInfo.getInputFilesList().stream()
            ...
            if (templateInput.getApplicationOutputName() != null) {
                ...
                try {
                    ...
                    if (applicationOutputName.contains("*")) {
                        expOutputData = experiment.getExperimentOutputsList().stream()
        ...
        parserInfo.getOutputFilesList().forEach(parserOutput -> {
    ...
    private void createParserDagRecursively(
        ...
        if (parentToChild.containsKey(parentParserInfo.getId())) {
            ...
            for (ParserConnector connector : parentToChild.get(parentParserInfo.getId())) {
                ...
                ParsingTaskInputs inputs = new ParsingTaskInputs();
                for (ParserConnectorInput connectorInput : connector.getConnectorInputsList()) {

                    Optional<ParserInput> parserInputOp = childParserInfo.getInputFilesList().stream()
                ...
                ParsingTaskOutputs outputs = new ParsingTaskOutputs();
                childParserInfo.getOutputFilesList().forEach(parserOutput -> {
                    ParsingTaskOutput output = new ParsingTaskOutput();
