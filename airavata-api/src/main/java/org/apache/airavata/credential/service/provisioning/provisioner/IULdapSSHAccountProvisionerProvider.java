...
import org.apache.airavata.credential.service.provisioning.SSHAccountProvisioner;
...
public class IULdapSSHAccountProvisionerProvider implements SSHAccountProvisionerProvider {
    ...
    public static final ConfigParam CANONICAL_SCRATCH_LOCATION = new ConfigParam("canonical-scratch-location")
            .setDescription(
                    "Pattern for scratch location. Use ${username} as replacement for username. For example, '/N/dc2/scratch/${username}/iu-gateway'.")
            .setOptional(false)
            .setType(ConfigParam.ConfigParamType.STRING);
    public static final ConfigParam CYBERGATEWAY_GROUP_DN = new ConfigParam("cybergateway-group-dn")
            ...
            .setOptional(false)
            .setType(ConfigParam.ConfigParamType.STRING);
    public static final List<ConfigParam> CONFIG_PARAMS = Arrays.asList(
            LDAP_HOST,
            LDAP_PORT,
            LDAP_USERNAME,
            LDAP_PASSWORD,
