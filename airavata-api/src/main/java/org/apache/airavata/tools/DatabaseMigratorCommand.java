...
* under the License.
*/
package org.apache.airavata.tools;
 ...
 *
 * <p>Usage: java -jar airavata-api.jar --spring.main.web-application-type=none
...
public class DatabaseMigratorCommand implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        if (jdbcURL == null || jdbcURL.isEmpty() || jdbcUser == null || jdbcUser.isEmpty() 
                || jdbcPwd == null || jdbcPwd.isEmpty() || currentAiravataVersion == null || currentAiravataVersion.isEmpty()) {
            logger.error(
                    "Missing required parameters. Usage: --migrate.enabled=true --migrate.url=... "
                            + "--migrate.user=... --migrate.password=... --migrate.version=...");
            logger.error("Or via command args: --url=... --user=... --pwd=... --version=...");
            System.exit(1);
        }

        logger.info("Starting database migration from version {} to {}", currentAiravataVersion, getIncrementedVersion(currentAiravataVersion));
    ...
    private void updateDB(String jdbcUrl, String jdbcUser, String jdbcPwd, String currentAiravataVersion) {
        ...
        try {
            ...
            if (migrationScriptPath == null || !Files.exists(migrationScriptPath)) {
                ...
                throw new RuntimeException("Migration script not found: " + MIGRATION_DIR + "/" + versionDir + "/" + migrationScriptName);
            }
            ...
            String selectQuery = "SELECT * FROM CONFIGURATION WHERE config_key='" + REGISTRY_VERSION + "' and category_id='SYSTEM'";
    ...
    private Path findMigrationScript(String versionDir, String scriptName) {
        ...
        for (String basePath : basePaths) {
            ...
            Path migrationPath = projectRoot.resolve(MIGRATION_DIR).resolve(versionDir).resolve(scriptName);
            ...
            if (Files.exists(apiPath)) {
                Path migrationPathFromApi = projectRoot.resolve(MIGRATION_DIR).resolve(versionDir).resolve(scriptName);
            ...
            for (int i = 0; i < 5; i++) {
                Path candidate = current.resolve(MIGRATION_DIR).resolve(versionDir).resolve(scriptName);
        ...
        for (int i = 0; i < 5; i++) {
            Path candidate = currentDir.resolve(MIGRATION_DIR).resolve(versionDir).resolve(scriptName);
            if (Files.exists(candidate)) {
                return candidate;
            }
