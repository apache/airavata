...
public interface DataReplicaLocationRepository extends JpaRepository<DataReplicaLocationEntity, String> {
    ...
    @Query("SELECT drl FROM DataReplicaLocationEntity drl WHERE drl.productUri = :productUri")
    List<DataReplicaLocationEntity> findByProductUri(@Param("productUri") String productUri);
}
