...
public class QueueOperator {
    ...
    private static final Logger logger = LoggerFactory.getLogger(QueueOperator.class);
    ...
    public QueueOperator(String helixClusterName, String instanceName, String zkConnectionString) throws Exception {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() -> helixManager.disconnect()));
    ...
    public synchronized String createQueue(String queueId, boolean monitor) throws InterruptedException {
        ...
        JobQueue.Builder jobQueueBuilder =
                new JobQueue.Builder(queueName).setWorkflowConfig(workflowCfgBuilder.build());

        taskDriver.start(jobQueueBuilder.build());

        if (monitor) {
            TaskState taskState = taskDriver.pollForWorkflowState(
                    queueName, TaskState.COMPLETED, TaskState.FAILED, TaskState.STOPPED, TaskState.ABORTED);
     ...
     */
    public synchronized String addTaskToQueue(String queueName, AbstractTask task, boolean globalParticipant)
        ...
        Map<String, String> paramMap = org.apache.airavata.execution.orchestrator.TaskUtil.serializeTaskData(task);
     ...
     */
    public synchronized String removeTaskFromQueue(String queueName, String taskId, int timeout)
