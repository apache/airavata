...
public class WorkflowOperator {
    ...
    private static final Logger logger = LoggerFactory.getLogger(WorkflowOperator.class);
    ...
    public synchronized String launchWorkflow(
            String processId, List<AbstractTask> tasks, boolean globalParticipant, boolean monitor) throws Exception {
        ...
        for (int i = 0; i < tasks.size(); i++) {
            ...
            TaskConfig.Builder taskBuilder = new TaskConfig.Builder()
                    ...
                    .setCommand(taskType);
            Map<String, String> paramMap = org.apache.airavata.execution.orchestrator.TaskUtil.serializeTaskData(data);
        ...
        // if the hfac that monitors a particular workflow, got killed due to some reason, who is taking the
        // responsibility

        if (monitor) {
            TaskState taskState = pollForWorkflowCompletion(workflow.getName(), 3600000);
            logger.info("Workflow " + workflowName + " for process " + processId + " finished with state "
                    + taskState.name());
        }
        return workflowName;
    }

    public synchronized TaskState pollForWorkflowCompletion(String workflowName, long timeout)
            throws InterruptedException {
        return taskDriver.pollForWorkflowState(
                workflowName, timeout, TaskState.COMPLETED, TaskState.FAILED, TaskState.STOPPED, TaskState.ABORTED);
    }
