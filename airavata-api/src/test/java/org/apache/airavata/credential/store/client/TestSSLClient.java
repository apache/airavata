...
public class TestSSLClient {
    private void invoke() {
        ...
        try {
            ...
            protocol = new TMultiplexedProtocol(protocol, "CredentialStore");
            //            transport = TSSLTransportFactory.getClientSocket(serverHost, serverPort, 10000, params);
            //            TProtocol protocol = new TBinaryProtocol(transport);
            CredentialStoreService.Iface client = new CredentialStoreService.Client(protocol);
    ...
    public static void testSSHCredential(CredentialStoreService.Iface client) {
        try {
    ...
    public static void testCertificateCredential(CredentialStoreService.Iface client) {
        try {
            ...
            System.out.println(
                    "gateway name  : " + credential.getCommunityUser().getGatewayName());
        } catch (TException | KeyStoreException | NoSuchAlgorithmException | CertificateException | IOException e) {
            e.printStackTrace();
        }
    }
