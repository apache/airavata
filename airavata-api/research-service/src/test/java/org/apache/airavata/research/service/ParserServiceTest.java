...
class ParserServiceTest {
    ...
    @BeforeEach
    void setUp() {
        parserService = new ParserService(appCatalogRegistry);
    ...
    void getParser_returnsParser() throws Exception {
        Parser parser = Parser.newBuilder().setId("parser-1").build();
        when(appCatalogRegistry.getParser("parser-1", "testGateway")).thenReturn(parser);
    ...
    void saveParser_returnsParserId() throws Exception {
        ...
        when(appCatalogRegistry.saveParser(parser)).thenReturn("parser-saved-id");

        String result = parserService.saveParser(ctx, parser);

        assertEquals("parser-saved-id", result);
        verify(appCatalogRegistry).saveParser(parser);
    }
    ...
    void listAllParsers_returnsList() throws Exception {
        ...
        when(appCatalogRegistry.listAllParsers("testGateway")).thenReturn(List.of(p1, p2));
    ...
    void removeParser_returnsTrue() throws Exception {
        doNothing().when(appCatalogRegistry).removeParser("parser-1", "testGateway");

        boolean result = parserService.removeParser(ctx, "parser-1", "testGateway");

        assertTrue(result);
        verify(appCatalogRegistry).removeParser("parser-1", "testGateway");
    }

    @Test
    void getParsingTemplate_returnsTemplate() throws Exception {
        ParsingTemplate template = ParsingTemplate.newBuilder().setId("tpl-1").build();
        when(appCatalogRegistry.getParsingTemplate("tpl-1", "testGateway")).thenReturn(template);
    ...
    void saveParsingTemplate_returnsTemplateId() throws Exception {
        ...
        when(appCatalogRegistry.saveParsingTemplate(template)).thenReturn("tpl-saved-id");
    ...
    void removeParsingTemplate_returnsTrue() throws Exception {
        doNothing().when(appCatalogRegistry).removeParsingTemplate("tpl-1", "testGateway");
    ...
    void getParsingTemplatesForExperiment_returnsList() throws Exception {
        ...
        when(appCatalogRegistry.getParsingTemplatesForExperiment("exp-1", "testGateway"))
    ...
    void getParser_wrapsRegistryException() throws Exception {
        when(appCatalogRegistry.getParser("bad-parser", "testGateway")).thenThrow(new RuntimeException("DB error"));
