...
import org.apache.airavata.research.config.ResearchProperties;
...
@Service
public class ResearchHubService {

    private static final Logger LOGGER = LoggerFactory.getLogger(ResearchHubService.class);
    ...
    public String spinRHubSession(String projectId, String sessionName) {
        String userId = UserContext.userId();
        int alreadyCreated = sessionService.countSessionsByUserIdAndStatus(userId, SessionStatusEnum.CREATED);
        ...
        ResearchProjectEntity project = researchProjectService.findProject(projectId);
        ArrayList<DatasetResourceEntity> datasetResourceArrayList =
                new ArrayList<DatasetResourceEntity>(project.getDatasetResources());

        SessionEntity session = sessionService.createSession(sessionName, project);
        ...
        for (DatasetResourceEntity datasetResource : datasetResourceArrayList) {
            spawnUrlBuilder
                    .append("&dataPath=")
