...
public class StringUtil {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (c.equals(delimeter)) {
                ...
                if (quote.equals(previousChar)) {
                    ...
                    currentItem += quote;
                    if (insideQuote) {
                        // mistakenly thought previous char was opening quote char, thus need to make
                        // this false
                        insideQuote = false;
                    } else {
                        // mistakenly thought previous char was closing quote char, thus need to make
                        // this true
                        insideQuote = true;
                    }
                } else {
    ...
    /**
     * Creates a new name by incrementing the number after the underscore at the end
     * of the old name. If there is no
    ...
    /**
     * Parses {@code --key=value} and {@code --key value} style arguments into a
     * map.
     * Keys or values containing dots are supported directly.
     */
    public static Map<String, String> parseCommandLineOptions(String[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            int eqPos = stripped.indexOf('=');
            if (eqPos != -1) {
                // --key=value
                result.put(stripped.substring(0, eqPos), stripped.substring(eqPos + 1));
            } else {
                // --key [value]
                boolean hasValue = (i + 1) < args.length && !args[i + 1].startsWith("-");
                result.put(stripped, hasValue ? args[++i] : "");
            }
        }
        return result;
    }
