...
public class ExecErrorRepository extends AbstractRepository<ExecErrorEntity, ExecErrorEntity, String> {
    ...
    public String updateProcessError(ErrorModel processError, String processId) throws Exception {
        return saveProcessError(processError, processId);
    }

    protected String saveProcessError(ErrorModel error, String processId) throws Exception {
    ...
    public String updateTaskError(ErrorModel taskError, String taskId) throws Exception {
        return saveTaskError(taskError, taskId);
    }

    protected String saveTaskError(ErrorModel error, String taskId) throws Exception {
        ExecErrorEntity entity = ExecutionMapper.INSTANCE.taskErrorToEntity(error);
        entity.setEntityType("TASK");
        entity.setCreationTime(AiravataUtils.getCurrentTimestamp());
        TaskEntity taskEntity = execute(em -> em.find(TaskEntity.class, taskId));
        if (taskEntity != null) {
