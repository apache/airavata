...
public interface ProfileMapper extends CommonMapperConversions {
    ...
    // (getEmailsList()/addAllEmails())
    // to the entity's `emails` property, so it silently drops every repeated-string
    // field on
    // UserProfile in both directions. Map them explicitly here (List<String> ->
    // repeated string
    ...
    default void userProfileRepeatedToModel(UserProfileEntity entity, @MappingTarget UserProfile.Builder builder) {
        if (entity.getEmails() != null)
            builder.addAllEmails(entity.getEmails());
        if (entity.getPhones() != null)
            builder.addAllPhones(entity.getPhones());
        if (entity.getNationality() != null)
            builder.addAllNationality(entity.getNationality());
        if (entity.getLabeledURI() != null)
            builder.addAllLabeledUri(entity.getLabeledURI());
    }
