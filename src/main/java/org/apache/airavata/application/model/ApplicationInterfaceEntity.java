...
* under the License.
*/
package org.apache.airavata.application.model;
...
public class ApplicationInterfaceEntity implements Serializable {
    ...
    @OneToMany(targetEntity = AppIoParamEntity.class, cascade = CascadeType.ALL, orphanRemoval = true, fetch = FetchType.EAGER)
    @JoinColumn(name = "INTERFACE_ID", referencedColumnName = "INTERFACE_ID")
    @SQLRestriction("DIRECTION = 'INPUT'")
    private List<AppIoParamEntity> applicationInputs;

    @OneToMany(targetEntity = AppIoParamEntity.class, cascade = CascadeType.ALL, orphanRemoval = true, fetch = FetchType.EAGER)
    @JoinColumn(name = "INTERFACE_ID", referencedColumnName = "INTERFACE_ID")
    @SQLRestriction("DIRECTION = 'OUTPUT'")
    private List<AppIoParamEntity> applicationOutputs;
    ...
    public List<AppIoParamEntity> getApplicationInputs() {
        return applicationInputs;
    }

    public void setApplicationInputs(List<AppIoParamEntity> applicationInputs) {
        this.applicationInputs = applicationInputs;
    }

    public List<AppIoParamEntity> getApplicationOutputs() {
        return applicationOutputs;
    }

    public void setApplicationOutputs(List<AppIoParamEntity> applicationOutputs) {
