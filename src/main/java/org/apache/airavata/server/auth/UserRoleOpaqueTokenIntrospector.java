...
/**
 * Wraps CILogon's token introspection response and populates authorities resolved
 * via {@link UserRoleLookupService} (looked up by username, not trusted from the
 * token), so that {@code @PreAuthorize(...)} works on service methods.
 *
...
public class UserRoleOpaqueTokenIntrospector implements OpaqueTokenIntrospector {
    ...
    public UserRoleOpaqueTokenIntrospector(
            OpaqueTokenIntrospector delegate, UserRoleLookupService userRoleLookupService) {
