...
public class ExperimentRepository extends AbstractRepository<ExperimentModel, ExperimentEntity, String> {
    ...
    public ExperimentRepository(ProcessRepository processRepository) {
    ...
    protected ExperimentModel toModel(ExperimentEntity entity) {
        return ExperimentMapper.INSTANCE.experimentToModel(entity);
    }

    @Override
    protected ExperimentEntity toEntity(ExperimentModel model) {
        return ExperimentMapper.INSTANCE.experimentToEntity(model);
    }

    protected String saveExperimentModelData(ExperimentModel experimentModel) throws Exception {
    ...
    protected ExperimentEntity saveExperiment(ExperimentModel experimentModel) throws Exception {
        ...
        if (!isExperimentExist(experimentId)) {
            ...
            // creation_time is immutable after create. This update path persists the full
            // model, and
            // callers may resend it with creation_time cleared — a clone, for instance,
            // re-sends the
            // experiment via update_experiment with an unset creation_time. longToTimestamp
            // maps 0 to a
        ...
        ExperimentEntity experimentEntity = ExperimentMapper.INSTANCE.experimentToEntity(experimentModel);
        ...
        // Persist the experiment row FIRST so the EXPERIMENT_ID exists before any child
        // row
        // (cascaded inputs/outputs/status/errors, and the user_configuration_data
        // below) references it.
        ExperimentEntity savedEntity = execute(entityManager -> entityManager.merge(experimentEntity));

        // UserConfigurationData and Processes are managed by their own entities (in
        // orchestration-service).
        // Persist UserConfigurationData via ExecutionDataAccess AFTER the experiment
        // row exists, since
    ...
    public String addExperiment(ExperimentModel experimentModel) throws Exception {
        ...
        ExperimentStatus experimentStatus = ExperimentStatus.newBuilder()
                ...
                .build();
        // experiment_id is a short, human-readable, URL-safe code (e.g. EXP-7QK2F9MX).
        // The experiment
        // name is kept separately in experiment_name. Regenerate on the rare collision
        // since it's the PK.
        String experimentId;
        do {
    ...
    public void updateExperiment(ExperimentModel updatedExperimentModel, String experimentId) throws Exception {
        saveExperimentModelData(updatedExperimentModel);
    }

    public ExperimentModel getExperiment(String experimentId) throws Exception {
        ExperimentModel baseModel = get(experimentId);
        ExperimentModel.Builder builder = baseModel.toBuilder();

        UserConfigurationDataModel ucdModel = processRepository.getUserConfigurationData(experimentId);
        if (ucdModel != null) {
            builder.setUserConfigurationData(ucdModel);
        }
        List<ProcessModel> processes = processRepository.getProcessesForExperiment(experimentId);
    ...
    public boolean isExperimentExist(String experimentId) throws Exception {
