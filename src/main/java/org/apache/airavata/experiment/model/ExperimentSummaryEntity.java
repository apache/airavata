...
/**
 * Read-only experiment summary, derived live from EXPERIMENT plus the latest
 * EXPERIMENT_STATUS and the
 * process scheduling host. Mapped with {@link Subselect} rather than a physical
 * EXPERIMENT_SUMMARY table:
 * the upstream summary was a DB view, but nothing populates a table, so the
 * dashboard "recent experiments"
 * search came back empty. A {@code @Subselect} entity has no table, so it is
 * excluded from {@code ddl-auto}
 * validation and always reflects current data. {@link Synchronize} flushes the
 * backing tables before reads.
 */
@Entity
@Immutable
