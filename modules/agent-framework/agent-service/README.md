...
See [Architecture](../../../README.md#architecture) for the full API layer overview. Agent API provides both HTTP endpoints (port 8090) and persistent gRPC channels (port 9090).
...
The Agent API provides HTTP endpoints for request/response operations. All endpoints are prefixed with `/api/v1/agents` and run on the HTTP Server (port 8090):
