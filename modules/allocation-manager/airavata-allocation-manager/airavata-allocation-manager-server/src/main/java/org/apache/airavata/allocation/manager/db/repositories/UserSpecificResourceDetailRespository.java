...
public class UserSpecificResourceDetailRespository
	...
	public UserSpecificResourceDetail getSpecificResource(long projectId, String specificResourceName)
			throws Exception {
		Map<String, Object> queryParameters = new HashMap<>();
		String query = "SELECT * from " + UserSpecificResourceDetailEntity.class.getSimpleName();
		query += " WHERE ";
		query += DBConstants.UserAllocationDetailTable.PROJECTID + " = " + projectId + " AND ";
		query += "specificResource" + " = " + specificResourceName;
		queryParameters.put(DBConstants.UserAllocationDetailTable.PROJECTID, projectId);
		queryParameters.put("specificResource", specificResourceName);
