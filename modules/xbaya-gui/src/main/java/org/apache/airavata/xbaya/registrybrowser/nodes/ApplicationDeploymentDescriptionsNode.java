...
public class ApplicationDeploymentDescriptionsNode extends AbstractAiravataTreeNode {
	...
	private void deleteApplicationDescription(JTree tree)
			throws Exception {
		if (askQuestion("Application descriptions", "Are you sure that you want to remove all application descriptions in this registry?")) {
			Registry registry = getRegistry();
			List<ApplicationDeploymentDescriptionWrap> descriptions = getApplicationDeploymentDescriptions().getDescriptions();
			for (ApplicationDeploymentDescriptionWrap descriptionWrap : descriptions) {
				registry.deleteDeploymentDescription(descriptionWrap.getService(), descriptionWrap.getHost(), descriptionWrap.getDescription().getId());
			}
			refresh();
			reloadTreeNode(tree, this);
		}
