...
* under the License.
*/
package org.apache.airavata.grpc.service;
...
import org.apache.airavata.agent.repository.ExperimentStorageCacheRepository;
...
public class AgentFileService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AgentFileService.class);
    ...
    private ExperimentStorageResponse fetchExperimentStorageFromAPI(String experimentId, String path) {
        ...
        if (responseEntity.getStatusCode().is2xxSuccessful()) {
            ...
            throw new IllegalStateException("Failed to fetch experiment storage: " + responseEntity.getStatusCode());
        }
    }
    ...
    private List<String> getUserExperimentIDs() {
        ...
        return Stream.iterate(0, offset -> offset + limit)
                .<List<ExperimentSummary>>map(offset -> {
                    try {
                ...
                .flatMap(List::stream)
                .map(ExperimentSummary::getExperimentId)
