...
public class SharingServiceComprehensiveTest extends ServiceIntegrationTestBase {
    ...
    public SharingServiceComprehensiveTest(SharingRegistryService sharingService, GatewayService gatewayService) {
    ...
    class DomainManagementTests {
        ...
        @Test
        @DisplayName("Should update domain")
        void shouldUpdateDomain() throws Exception {
            // Given
            Domain domain = sharingService.getDomain(testDomainId);
            domain.setDescription("Updated description");

            // When
            boolean updated = sharingService.updateDomain(domain);
    ...
    class UserLifecycleTests {
        ...
        @Test
        @DisplayName("Should update user and related single-user group")
        void shouldUpdateUserAndSingleUserGroup() throws Exception {
            // Given
            User user = createUser("update-user", "Original", "Name", "original@test.com");
            sharingService.createUser(user);

            // When
            user.setFirstName("Updated");
            user.setLastName("FullName");
            sharingService.updatedUser(user);

            // Then
            User retrieved = sharingService.getUser(testDomainId, "update-user");
            assertEquals("Updated", retrieved.getFirstName());
            assertEquals("FullName", retrieved.getLastName());
        }
        ...
        @Test
        @DisplayName("Should list users in domain")
        void shouldListUsersInDomain() throws Exception {
            // Given
            sharingService.createUser(createUser("list-user-1", "User", "One", "u1@test.com"));
            sharingService.createUser(createUser("list-user-2", "User", "Two", "u2@test.com"));

            // When
            List<User> users = sharingService.getUsers(testDomainId, 0, -1);
    ...
    class GroupManagementTests {
        ...
        void shouldAddUsersToGroup() throws Exception {
            ...
            // When - Add users
            boolean added = sharingService.addUsersToGroup(testDomainId, List.of("member-1", "member-2"), "member-test-group");
    ...
    class EntityAndSharingTests {
        ...
        void shouldCreateEntityWithOwnerAccess() throws Exception {
            ...
            entity.setOriginalEntityCreationTime(AiravataUtils.getUniqueTimestamp().getTime());
            ...
            Entity retrieved = sharingService.getEntity(testDomainId, "test-entity-1");
        ...
        void shouldCascadePermissionsToChildEntities() throws Exception {
            ...
            parent.setOriginalEntityCreationTime(AiravataUtils.getUniqueTimestamp().getTime());
            ...
            child.setOriginalEntityCreationTime(AiravataUtils.getUniqueTimestamp().getTime());
        ...
        void shouldGetSharedCountForEntity() throws Exception {
            ...
            // Initially
            Entity initial = sharingService.getEntity(testDomainId, "count-entity");
            ...
            // Then
            Entity afterShare = sharingService.getEntity(testDomainId, "count-entity");
    ...
    class EntitySearchTests {
        ...
        void shouldSearchEntitiesByName() throws Exception {
            ...
            Entity entity1 = createEntity("search-entity-1", "SEARCH_TYPE", "search-owner", "Alpha Project");
            Entity entity2 = createEntity("search-entity-2", "SEARCH_TYPE", "search-owner", "Beta Project");
            ...
            List<Entity> results = sharingService.searchEntities(testDomainId, "search-owner", filters, 0, -1);
        ...
        void shouldSearchEntitiesByFullText() throws Exception {
            ...
            Entity entity = createEntity("ft-entity", "FT_TYPE", "fulltext-owner", "Gaussian Experiment");
            ...
            List<Entity> results = sharingService.searchEntities(testDomainId, "fulltext-owner", filters, 0, -1);
        ...
        void shouldSearchEntitiesByEntityType() throws Exception {
            ...
            Entity entityA = createEntity("type-a-entity", "TYPE_A", "type-search-owner", "Type A Entity");
            Entity entityB = createEntity("type-b-entity", "TYPE_B", "type-search-owner", "Type B Entity");
            ...
            List<Entity> results = sharingService.searchEntities(testDomainId, "type-search-owner", filters, 0, -1);
    ...
    private Entity createEntity(String entityId, String entityTypeId, String ownerId, String name) {
        Entity entity = new Entity();
