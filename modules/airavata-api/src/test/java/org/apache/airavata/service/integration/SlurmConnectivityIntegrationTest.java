 ...
 * 
 * <p><b>Note:</b> On ARM64 systems (Apple Silicon), these tests run under emulation and are
 * significantly slower. Set system property {@code skip.slurm.tests=true} to skip on ARM64.
 */
@DisplayName("SLURM Connectivity Integration Tests")
@Timeout(value = 5, unit = TimeUnit.MINUTES)  // Extended timeout for ARM64 emulation
public class SlurmConnectivityIntegrationTest {
    ...
    void setUp() throws IOException {
        // Check if SLURM tests should be skipped (useful on ARM64 where emulation is slow)
        ...
        // Set connection timeout - longer for ARM64 emulation
        boolean isArm64 = TestcontainersConfig.isArm64Architecture();
        sshClient.setConnectTimeout(isArm64 ? 30000 : 15000);  // 30s on ARM64, 15s otherwise
        sshClient.setTimeout(isArm64 ? 60000 : 30000);          // 60s on ARM64, 30s otherwise
        
        // Retry SSH connection with exponential backoff
        // More retries on ARM64 due to emulation delays
        int maxRetries = isArm64 ? 10 : 5;
        int retryDelayMs = isArm64 ? 5000 : 2000;
        ...
        logger.warn("Could not connect to SLURM container after {} attempts: {}. Skipping SLURM tests.", 
                maxRetries, lastException != null ? lastException.getMessage() : "unknown error");
        assumeTrue(false, "Could not connect to SLURM container: " + 
    ...
    class SlurmCommandTests {
        ...
        void shouldSubmitAndCancelJob() throws IOException {
            ...
            String jobId = null;
            try (Session session = sshClient.startSession()) {
                Session.Command cmd = session.exec("sbatch /tmp/test_job.sh 2>&1 || echo 'Job submission not available'");
                String output = IOUtils.readFully(cmd.getInputStream()).toString().trim();
                cmd.join(10, TimeUnit.SECONDS);
