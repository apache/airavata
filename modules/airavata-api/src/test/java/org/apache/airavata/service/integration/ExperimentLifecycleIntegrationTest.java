...
public class ExperimentLifecycleIntegrationTest extends ServiceIntegrationTestBase {
    ...
    void shouldCompleteFullExperimentLifecycle() throws Exception {
        ...
        ExperimentModel experiment = TestDataFactory.createTestExperiment(
                "Lifecycle Test Experiment", projectId, TEST_GATEWAY_ID);
        ...
        assertThat(finalProcess.getProcessStatuses().stream()
                .anyMatch(s -> s.getState() == ProcessState.COMPLETED)).isTrue();
    ...
    void shouldVerifyMessagesPublishedDuringLifecycle() throws Exception {
        ...
        MessageHandler handler = MessageVerificationUtils.createCapturingHandlerWithLatch(
                capturedMessages, messagesReceived, 3);
        ...
        try {
            ...
            // Create experiment
            ExperimentModel experiment = TestDataFactory.createTestExperiment(
                    "Message Test Experiment", projectId, TEST_GATEWAY_ID);
    ...
    void shouldVerifyExperimentStateTransitionsPersisted() throws RegistryServiceException {
        ...
        ExperimentModel experiment = TestDataFactory.createTestExperiment(
                "State Transition Experiment", projectId, TEST_GATEWAY_ID);
        String experimentId = registryService.createExperiment(TEST_GATEWAY_ID, experiment);
        commitTransaction();
        ...
        for (ExperimentState expectedState : states) {
            boolean found = retrieved.getExperimentStatus().stream()
                    .anyMatch(s -> s.getState() == expectedState);
