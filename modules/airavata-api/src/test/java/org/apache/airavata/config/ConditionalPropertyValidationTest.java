...
public class ConditionalPropertyValidationTest {
    ...
    void testSchedulerPolicyPropertiesAreBound() {
        assertNotNull(properties.services().scheduler().selectionPolicy(), 
            "Scheduler selection-policy should be configured");
        assertNotNull(properties.services().scheduler().reschedulerPolicy(), 
            "Scheduler rescheduler-policy should be configured");
        
        // Verify default values
        assertEquals("DefaultComputeResourceSelectionPolicy", 
            properties.services().scheduler().selectionPolicy(),
            "Default selection policy should be DefaultComputeResourceSelectionPolicy");
        assertEquals("ExponentialBackOffReScheduler", 
    ...
    void testMessagingEnabledFlags() {
        ...
        assertFalse(properties.rabbitmq().enabled(), 
            "rabbitmq.enabled should be false in test");
        assertFalse(properties.kafka().enabled(), 
            "kafka.enabled should be false in test");
    }
    ...
    void testFlywayDisabled() {
        assertFalse(properties.flyway().enabled(), 
            "flyway.enabled should be false in test");
    }
    ...
    void testPropertyNamingConventions() {
        ...
        assertNotNull(properties.services().monitor().compute(), 
            "services.monitor.compute should be accessible");
        assertNotNull(properties.services().monitor().email(), 
            "services.monitor.email should be accessible");
        assertNotNull(properties.services().monitor().realtime(), 
            "services.monitor.realtime should be accessible");
        
        // Verify the enabled flags are accessible
        assertTrue(properties.services().monitor().compute().enabled(),
    ...
    void testAllConditionalPropertiesExist() {
        // Core services
        assertNotNull(properties.services().controller(),
            "services.controller must be set");
        assertNotNull(properties.services().participant(),
            "services.participant must be set");
        
        // Workflow managers
        assertNotNull(properties.services().prewm(),
            "services.prewm must be set");
        assertNotNull(properties.services().postwm(),
            "services.postwm must be set");
        assertNotNull(properties.services().parser(),
            "services.parser must be set");
        
        // Monitor services
        assertNotNull(properties.services().monitor().compute(),
            "services.monitor.compute must be set");
        assertNotNull(properties.services().monitor().email(),
            "services.monitor.email must be set");
        assertNotNull(properties.services().monitor().realtime(),
            "services.monitor.realtime must be set");
        
        // Scheduler services
        assertNotNull(properties.services().scheduler(),
            "services.scheduler must be set");
        assertNotNull(properties.services().scheduler().interpreter(),
            "services.scheduler.interpreter must be set");
        assertNotNull(properties.services().scheduler().rescheduler(),
            "services.scheduler.rescheduler must be set");
        
        // Telemetry
        assertNotNull(properties.services().telemetry(),
            "services.telemetry must be set");
        
        // API services
        assertNotNull(properties.services().rest(),
            "services.rest must be set");
        assertNotNull(properties.services().thrift(),
            "services.thrift must be set");
        
        // Messaging
        assertNotNull(properties.rabbitmq(),
            "rabbitmq must be set");
        assertNotNull(properties.kafka(),
            "kafka must be set");
        
        // Database migration
        assertNotNull(properties.flyway(),
            "flyway must be set");
        
        // Security
        assertNotNull(properties.security().iam(),
            "security.iam must be set");
        assertNotNull(properties.security().authentication(),
            "security.authentication must be set");
        assertNotNull(properties.security().authzCache(),
