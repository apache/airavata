...
/**
 * MapStruct mapper for converting between ParserInputEntity and ParserInput.
 */
@Mapper(componentModel = "spring", config = EntityMapperConfig.class)
public interface ParserInputMapper {

    @Mapping(target = "type", qualifiedByName = "stringToIOType")
    ParserInput toModel(ParserInputEntity entity);

    @Mapping(target = "type", qualifiedByName = "ioTypeToString")
    @Mapping(target = "parser", ignore = true) // Set by parent entity
    ParserInputEntity toEntity(ParserInput model);
    ...
    List<ParserInput> toModelList(List<ParserInputEntity> entities);

    List<ParserInputEntity> toEntityList(List<ParserInput> models);
}
