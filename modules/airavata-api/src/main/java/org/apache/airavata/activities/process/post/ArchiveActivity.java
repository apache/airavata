...
public class ArchiveActivity implements WorkflowActivity {
    ...
    public String run(WorkflowActivityContext ctx) {
        ...
        logger.info("ArchiveActivity for process {}", input.processId());
        ...
        try {
            ...
            var task = factory.createArchiveTask(input.processId());
            return TaskExecutorHelper.executeTask(task, input);
        } catch (Exception e) {
            logger.error("ArchiveActivity failed for process {}", input.processId(), e);
            throw new RuntimeException("ArchiveActivity failed: " + e.getMessage(), e);
        }
    }
}
