...
* under the License.
*/
package org.apache.airavata.workflow.monitoring.data;
...
/**
 * Workflow for scheduled data analysis.
 */
public class DataAnalyzerWorkflow implements Workflow {

    private static final Logger logger = LoggerFactory.getLogger(DataAnalyzerWorkflow.class);

    @Override
    public WorkflowStub create() {
        return ctx -> {
            var input = ctx.getInput(ScheduledWorkflowInput.class);
            logger.info("DataAnalyzerWorkflow started: interval={}s, jobId={}", input.intervalSeconds(), input.jobId());

            var activityInput = new ScheduledActivityInput(input.jobId(), input.parallelJobs());

            while (true) {
                try {
                    logger.debug("Executing DataAnalyzerActivity for jobId {}", input.jobId());
                    ctx.callActivity(DataAnalyzerActivity.class.getName(), activityInput, Void.class)
                            .await();
                    ...
                    logger.error("Error in DataAnalyzerWorkflow for jobId {}: {}", input.jobId(), e.getMessage(), e);
