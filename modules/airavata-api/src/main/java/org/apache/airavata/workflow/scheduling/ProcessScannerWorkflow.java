...
/**
 * Workflow for scheduled process scanning.
 */
public class ProcessScannerWorkflow implements Workflow {

    private static final Logger logger = LoggerFactory.getLogger(ProcessScannerWorkflow.class);
    ...
    public WorkflowStub create() {
        return ctx -> {
            ...
            logger.info(
                    "ProcessScannerWorkflow started: interval={}s, jobId={}", input.intervalSeconds(), input.jobId());

            var activityInput = new ScheduledActivityInput(input.jobId(), input.parallelJobs());

            while (true) {
                try {
                    logger.debug("Executing ProcessScannerActivity for jobId {}", input.jobId());
                    ctx.callActivity(ProcessScannerActivity.class.getName(), activityInput, Void.class)
                            .await();
                    ...
                    logger.error("Error in ProcessScannerWorkflow for jobId {}: {}", input.jobId(), e.getMessage(), e);
