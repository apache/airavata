...
public class CompletingActivity implements WorkflowActivity {

    private static final Logger logger = LoggerFactory.getLogger(CompletingActivity.class);

    @Override
    public String run(WorkflowActivityContext ctx) {
        var input = ctx.getInput(BaseActivityInput.class);
        logger.info("CompletingActivity for process {}", input.processId());
        ...
        try {
            ...
            var task = factory.createCompletingTask(input.processId());
            return TaskExecutorHelper.executeTask(task, input);
        } catch (Exception e) {
            logger.error("CompletingActivity failed for process {}", input.processId(), e);
            throw new RuntimeException("CompletingActivity failed: " + e.getMessage(), e);
        }
    }
}
