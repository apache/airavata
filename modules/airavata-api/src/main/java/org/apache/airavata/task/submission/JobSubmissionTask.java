...
public abstract class JobSubmissionTask extends AiravataTask {
    ...
    protected JobSubmissionOutput submitBatchJob(
        ...
        // Record job submission for cluster monitoring
        if (computeSubmissionTracker != null && jsoutput.getJobId() != null && !jsoutput.isJobSubmissionFailed()) {
            String computeResourceId = getTaskContext().getComputeResourceId();
    ...
    private CommandOutput submitCommandWithRecording(
        ...
        if (commandOutput.getStdOut() == null || "".equals(commandOutput.getStdOut())) {
            ...
            CommandOutput recordingFileReadCommandOutput = agentAdaptor.executeCommand(
                    "cat " + getJobCommandRecordingFile(groovyMapData), groovyMapData.getWorkingDirectory());
    ...
    protected void addMonitoringCommands(GroovyMapData mapData) {
        try {
            ...
            if (ctx != null) {
                var props = ctx.getBean(AiravataServerProperties.class);
                String url = props.services().monitor().compute().jobStatusCallbackUrl();
                if (props.services().monitor().realtime().enabled() && url != null && !url.isBlank()) {
