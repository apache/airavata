...
public class Dispatcher {
    ...
    private void dispatchTenantEvent(CrudType crudType, Object domainModel) throws Exception {
        var gateway = (Gateway) domainModel;
    ...
    private void dispatchUserProfileEvent(CrudType crudType, Object domainModel) throws Exception {
        var userProfile = (UserProfile) domainModel;
        ...
        switch (crudType) {
            case CREATE -> {
                ...
                try {
                    var user = convertUserProfileToUser(userProfile);
    ...
    private void dispatchProjectEvent(CrudType crudType, Object domainModel) throws Exception {
        var project = (Project) domainModel;
        var domainId = project.getGatewayId();
        var entityId = project.getProjectID();
    ...
    private void createEntityTypesForDomain(String domainId) {
        ...
        for (var entityTypeName : entityTypeNames) {
            try {
                var entityTypeId = domainId + ":" + entityTypeName;
                if (!sharingRegistryService.isEntityTypeExists(domainId, entityTypeId)) {
                    var entityType = new org.apache.airavata.sharing.model.EntityType();
    ...
    private void createPermissionTypesForDomain(String domainId) {
        ...
        for (var permissionName : permissionNames) {
            try {
                var permissionTypeId = domainId + ":" + permissionName;
