...
public class JobSubmissionActivity implements WorkflowActivity {

    private static final Logger logger = LoggerFactory.getLogger(JobSubmissionActivity.class);

    @Override
    public String run(WorkflowActivityContext ctx) {
        var input = ctx.getInput(BaseActivityInput.class);
        logger.info("JobSubmissionActivity for process {}", input.processId());
        ...
        try {
            ...
            var task = factory.createJobSubmissionTask(input.processId());
            return TaskExecutorHelper.executeTask(task, input);
        } catch (Exception e) {
            logger.error("JobSubmissionActivity failed for process {}", input.processId(), e);
            throw new RuntimeException("JobSubmissionActivity failed: " + e.getMessage(), e);
        }
    }
}
