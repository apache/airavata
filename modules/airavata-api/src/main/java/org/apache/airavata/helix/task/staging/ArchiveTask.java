...
* under the License.
*/
package org.apache.airavata.helix.task.staging;
        ...
        try {
            ...
            try {
                ...
                if (sourceURI.getPath().endsWith("/")) {
                    tarDirPath =
                            sourceURI.getPath().substring(0, sourceURI.getPath().length() - 1);
                } else {
                ...
                destFilePath = buildDestinationFilePath(outputStorageRoot, archiveFileName);
            ...
            try {
                ...
                if (fileMetadata.getSize() < maxArchiveSize) {
                    boolean fileTransferred = transferFileToStorage(
                            tarCreationAbsPath, destFilePath, archiveFileName, adaptor, storageResourceAdaptor);
                    if (!fileTransferred) {
                        logger.error("Failed to transfer created archive file " + tarCreationAbsPath);
                        throw new TaskOnFailException(
                                "Failed to transfer created archive file " + tarCreationAbsPath, false, null);
                    }
                    ...
                    try {
                        for (String command : unarchiveCommands) {
                            ...
                            CommandOutput unTarCommandOutput =
                                    storageResourceAdaptor.executeCommand(command, destParent);
                    ...
                    logger.error(
                            "Archive size {} MB is larger than the maximum allowed size {} MB. So skipping the transfer.",
