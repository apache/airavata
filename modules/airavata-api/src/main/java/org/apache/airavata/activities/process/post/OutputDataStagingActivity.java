...
public class OutputDataStagingActivity implements WorkflowActivity {

    private static final Logger logger = LoggerFactory.getLogger(OutputDataStagingActivity.class);

    @Override
    public String run(WorkflowActivityContext ctx) {
        var input = ctx.getInput(BaseActivityInput.class);
        logger.info("OutputDataStagingActivity for process {}", input.processId());
        ...
        try {
            ...
            var task = factory.createOutputDataStagingTask(input.processId());
            return TaskExecutorHelper.executeTask(task, input);
        } catch (Exception e) {
            logger.error("OutputDataStagingActivity failed for process {}", input.processId(), e);
            throw new RuntimeException("OutputDataStagingActivity failed: " + e.getMessage(), e);
        }
    }
}
