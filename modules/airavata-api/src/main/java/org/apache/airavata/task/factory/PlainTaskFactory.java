...
import org.apache.airavata.task.submission.ForkJobSubmissionTask;
...
/**
 * Task factory for PLAIN compute resources (SSH-only access, no job scheduler).
 * Uses ForkJobSubmissionTask to execute commands directly via SSH.
 */
@Component
@ConditionalOnParticipant
public class PlainTaskFactory implements DaprTaskFactory {

    private static final Logger LOGGER = LoggerFactory.getLogger(PlainTaskFactory.class);

    private final ApplicationContext applicationContext;

    public PlainTaskFactory(ApplicationContext applicationContext) {
        this.applicationContext = applicationContext;
    }

    @Override
    public AiravataTask createEnvSetupTask(String processId) {
        LOGGER.info("Creating Plain EnvSetupTask for process {}...", processId);
        return applicationContext.getBean(EnvSetupTask.class);
    }

    @Override
    public AiravataTask createInputDataStagingTask(String processId) {
        LOGGER.info("Creating Plain InputDataStagingTask for process {}...", processId);
        return applicationContext.getBean(InputDataStagingTask.class);
    }

    @Override
    public AiravataTask createJobSubmissionTask(String processId) {
        LOGGER.info("Creating Plain ForkJobSubmissionTask for process {}...", processId);
        return applicationContext.getBean(ForkJobSubmissionTask.class);
    }

    @Override
    public AiravataTask createOutputDataStagingTask(String processId) {
        LOGGER.info("Creating Plain OutputDataStagingTask for process {}...", processId);
        return applicationContext.getBean(OutputDataStagingTask.class);
    }

    @Override
    public AiravataTask createArchiveTask(String processId) {
        LOGGER.info("Creating Plain ArchiveTask for process {}...", processId);
        return applicationContext.getBean(ArchiveTask.class);
    }

    @Override
    public AiravataTask createJobVerificationTask(String processId) {
        LOGGER.info("Creating Plain JobVerificationTask for process {}...", processId);
        return applicationContext.getBean(JobVerificationTask.class);
    }

    @Override
    public AiravataTask createCompletingTask(String processId) {
        LOGGER.info("Creating Plain CompletingTask for process {}...", processId);
        return applicationContext.getBean(CompletingTask.class);
    }

    @Override
    public AiravataTask createParsingTriggeringTask(String processId) {
        LOGGER.info("Creating Plain ParsingTriggeringTask for process {}...", processId);
