...
public class CancelCompletingActivity implements WorkflowActivity {

    private static final Logger logger = LoggerFactory.getLogger(CancelCompletingActivity.class);

    @Override
    public String run(WorkflowActivityContext ctx) {
        var input = ctx.getInput(BaseActivityInput.class);
        logger.info("CancelCompletingActivity for process {}", input.processId());

        try {
            var task = WorkflowRuntimeHolder.getBean(CancelCompletingTask.class);
            return TaskExecutorHelper.executeTask(task, input);
        } catch (Exception e) {
            logger.error("CancelCompletingActivity failed for process {}", input.processId(), e);
            throw new RuntimeException("CancelCompletingActivity failed: " + e.getMessage(), e);
        }
    }
}
