...
* under the License.
*/
package org.apache.airavata.activities.process.pre;
...
public class InputDataStagingActivity implements WorkflowActivity {

    private static final Logger logger = LoggerFactory.getLogger(InputDataStagingActivity.class);

    @Override
    public String run(WorkflowActivityContext ctx) {
        var input = ctx.getInput(BaseActivityInput.class);
        logger.info("InputDataStagingActivity for process {}", input.processId());
        ...
        try {
            ...
            var task = factory.createInputDataStagingTask(input.processId());
            return TaskExecutorHelper.executeTask(task, input);
        } catch (Exception e) {
            logger.error("InputDataStagingActivity failed for process {}", input.processId(), e);
            throw new RuntimeException("InputDataStagingActivity failed: " + e.getMessage(), e);
        }
    }
}
