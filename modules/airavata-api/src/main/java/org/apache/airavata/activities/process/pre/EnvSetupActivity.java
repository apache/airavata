...
public class EnvSetupActivity implements WorkflowActivity {

    private static final Logger logger = LoggerFactory.getLogger(EnvSetupActivity.class);

    @Override
    public String run(WorkflowActivityContext ctx) {
        var input = ctx.getInput(BaseActivityInput.class);
        logger.info("EnvSetupActivity for process {}", input.processId());
        ...
        try {
            ...
            var task = factory.createEnvSetupTask(input.processId());
            return TaskExecutorHelper.executeTask(task, input);
        } catch (Exception e) {
            logger.error("EnvSetupActivity failed for process {}", input.processId(), e);
            throw new RuntimeException("EnvSetupActivity failed: " + e.getMessage(), e);
        }
    }
}
