...
* under the License.
*/
package org.apache.airavata.workflow.monitoring.realtime;
...
import org.apache.airavata.orchestrator.internal.monitoring.DaprMonitoringHandler;
...
public class RealtimeMonitorHandler extends ServerLifecycle implements DaprMonitoringHandler {

    private static final Logger logger = LoggerFactory.getLogger(RealtimeMonitorHandler.class);
    ...
    private void process(String key, String value) throws MonitoringException {
        logger.info("received post from {} on {}: {}->{}", publisherId, DaprTopics.MONITORING, key, value);
    ...
    protected void doStart() throws Exception {
        logger.info("Starting RealtimeMonitorHandler (Dapr Pub/Sub) for topic: {}", DaprTopics.MONITORING);
        started = true;
        logger.info("RealtimeMonitorHandler started; Dapr delivers to /api/v1/dapr/pubsub/monitoring-data-topic");
    }

    @Override
    protected void doStop() throws Exception {
        logger.info("Stopping RealtimeMonitorHandler");
        started = false;
        logger.info("RealtimeMonitorHandler stopped");
    }
