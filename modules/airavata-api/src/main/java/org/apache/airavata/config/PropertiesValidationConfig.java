...
public class PropertiesValidationConfig {
    ...
    public PropertiesValidationConfig(AiravataServerProperties properties) {
        ...
        if (properties == null) {
            throw new IllegalStateException(
                "CRITICAL: AiravataServerProperties bean is null. " +
        ...
        if (dbConfig instanceof Record record) {
            for (RecordComponent component : record.getClass().getRecordComponents()) {
                try {
                    ...
                    if ("url".equals(component.getName()) && (value == null || value.toString().isBlank())) {
    ...
    private void logConfiguration() {
        ...
        if (properties.services() != null) {
            ...
            logger.info("    thrift.enabled = {}", services.thrift() != null && services.thrift().enabled());
            logger.info("    rest.enabled = {}", services.rest() != null && services.rest().enabled());
            logger.info("    controller.enabled = {}", services.controller() != null && services.controller().enabled());
            logger.info("    participant.enabled = {}", services.participant() != null && services.participant().enabled());
            ...
            if (services.monitor() != null) {
                if (services.monitor().email() != null) {
                    logger.info("    monitor.email.enabled = {}", services.monitor().email().enabled());
                }
                if (services.monitor().compute() != null) {
                    logger.info("    monitor.compute.enabled = {}", services.monitor().compute().enabled());
                }
            }
        }
        ...
        if (properties.security() != null) {
            ...
            if (properties.security().authzCache() != null) {
                logger.info("    authzCache.enabled = {}", properties.security().authzCache().enabled());
            }
            if (properties.security().iam() != null) {
                logger.info("    iam.enabled = {}", properties.security().iam().enabled());
                logger.info("    iam.server-url = {}", properties.security().iam().serverUrl());
            }
        }
