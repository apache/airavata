...
/**
 * Unified InputDataEntity that consolidates input data records from experiments, processes,
 * applications, and handlers.
 *
 * <p>This entity replaces the following separate entities:
 * <ul>
 *   <li>{@code ExperimentInputEntity}</li>
 *   <li>{@code ProcessInputEntity}</li>
 *   <li>{@code ApplicationInputEntity}</li>
 *   <li>{@code HandlerInputEntity}</li>
 * </ul>
 *
 * <p>The {@code parentType} field discriminates between different input sources,
 * while {@code parentId} stores the ID of the parent entity (experiment ID, process ID, etc.).
 */
@Entity(name = "InputDataEntity")
@Table(
        name = "INPUT_DATA",
        indexes = {
            @Index(name = "idx_input_parent", columnList = "PARENT_ID, PARENT_TYPE"),
            @Index(name = "idx_input_name", columnList = "INPUT_NAME")
        })
@IdClass(InputDataEntityPK.class)
public class InputDataEntity implements Serializable {
    ...
    public boolean equals(Object obj) {
        ...
        InputDataEntity that = (InputDataEntity) obj;
