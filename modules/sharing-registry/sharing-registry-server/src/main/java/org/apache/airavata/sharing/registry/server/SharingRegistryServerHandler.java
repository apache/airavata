...
public class SharingRegistryServerHandler implements SharingRegistryService.Iface {
    ...
    public SharingRegistryServerHandler() throws ApplicationSettingsException, TException, IOException {
        this(new SharingRegistryDBInitConfig());
    }

    public SharingRegistryServerHandler(SharingRegistryDBInitConfig sharingRegistryDBInitConfig) throws ApplicationSettingsException, TException, IOException {
    ...
    public boolean synchronizeWithCustos(String gatewayId, String custosId) throws TException {
        try {
            ...
            logger.info("Calling synchronize with custos " + gatewayId);

            List<EntityType> entityTypeList = getEntityTypes(gatewayId, 0, -1);
            for (EntityType entityType : entityTypeList) {
                String id = entityType.getEntityTypeId().split(gatewayId + ":")[1];
                String description = entityType.getDescription();
                String name = entityType.getName();

                org.apache.custos.sharing.service.EntityType entityTypeCustos = org.apache.custos.sharing.service.EntityType.newBuilder()
                        .setName(name)
                        .setDescription(description)
                        .setId(id)
                        .build();
                org.apache.custos.sharing.service.EntityType permType = sharingManagementClient.getEntityType(custosId, entityTypeCustos);
                if (permType.getName() == null || permType.getName().trim().equals("")) {
                    sharingManagementClient.createEntityType(custosId, entityTypeCustos);
                    logger.info("Entity type " + entityTypeCustos.getId() + " created");
                }
            }

            List<PermissionType> permissionTypeList = getPermissionTypes(gatewayId, 0, -1);

            for (PermissionType permissionType : permissionTypeList) {
                String id = permissionType.permissionTypeId.split(gatewayId + ":")[1];
                String description = permissionType.getDescription();
                String name = permissionType.getName();

                org.apache.custos.sharing.service.PermissionType entityTypeCustos = org.apache.custos.sharing.service.PermissionType.newBuilder()
                        .setName(name)
                        .setDescription(description)
                        .setId(id)
                        .build();
                org.apache.custos.sharing.service.PermissionType permType = sharingManagementClient.getPermissionType(custosId, entityTypeCustos);
                if (permType.getName() == null || permType.getName().trim().equals("")) {
                    sharingManagementClient.createPermissionType(custosId, entityTypeCustos);
                    logger.info("Permission  type " + entityTypeCustos.getId() + " created");
                }

            }
            createCustosSharings(gatewayId, custosId, null);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return true;
    ...
    private boolean createCustosSharings(String gatewayId, String custosId, String parentId) {
        try {
            HashMap<String, String> filters = new HashMap<>();
            filters.put(DBConstants.PermissionTypeTable.DOMAIN_ID, gatewayId);
            List<PermissionType> permissionTypes = (new PermissionTypeRepository()).select(filters, 0, -1);


            List<Entity> entities = (new EntityRepository()).getChildEntities(gatewayId, parentId);
            entities.forEach(ent -> {
                String ownerId = ent.ownerId.split("@" + gatewayId)[0];
                String typeId = ent.entityTypeId.split(gatewayId + ":")[1];
                logger.info("Entity Id:  " + ent.entityId + "Parent Id: " + parentId + " owner Id:  " + ownerId);


                org.apache.custos.sharing.service.Entity entity = org.apache.custos.sharing.service.Entity.newBuilder()
                        .setOwnerId(ownerId)
                        .setName(ent.getName())
                        .setId(ent.getEntityId())
                        .setDescription(ent.getDescription() != null ? ent.getDescription() : "")
                        .setType(typeId).build();

                if (parentId != null) {

                    entity = entity.toBuilder().setParentId(parentId).build();

                }


                sharingManagementClient.createEntity(custosId, entity);
                logger.info("Entity   " + entity.getId() + " created Parent Id" + entity.getParentId());

                org.apache.custos.sharing.service.Entity finalEntity = entity;

                permissionTypes.forEach(permType -> {
                    try {
                        String custosPermType = permType.permissionTypeId.split(gatewayId + ":")[1];
                        if (!permType.permissionTypeId.equals(gatewayId + ":" + "OWNER")) {

                            List<User> users = (new UserRepository()).getAccessibleUsersInternal(gatewayId, ent.entityId, permType.permissionTypeId, SharingType.DIRECT_CASCADING);
                            users.forEach(user -> {
                                String userId = user.getUserId().split("@" + gatewayId)[0];

                                org.apache.custos.sharing.service.PermissionType type = org.apache.custos.sharing.service.PermissionType.newBuilder().
                                        setId(custosPermType).build();

                                SharingRequest request = SharingRequest.newBuilder()
                                        .setEntity(finalEntity)
                                        .setPermissionType(type)
                                        .addAllOwnerId(Arrays.asList(userId))
                                        .setCascade(true).build();

                                sharingManagementClient.shareEntityWithUsers(custosId, request);
                                logger.info("Entity   " + finalEntity.getId() + " shared with user " + userId + " Perm type " + type.getId() + "Cascading True");


                            });
                            List<User> NonDirectusers = (new UserRepository()).
                                    getAccessibleUsersInternal(gatewayId, ent.entityId, permType.permissionTypeId, SharingType.DIRECT_NON_CASCADING);
                            NonDirectusers.forEach(user -> {
                                String userId = user.getUserId().split("@" + gatewayId)[0];
                                ...
                                SharingRequest request = SharingRequest.newBuilder()
                                        ...
                                        .setPermissionType(type)
                                        .addAllOwnerId(Arrays.asList(userId))
                                        .setCascade(false).build();

                                sharingManagementClient.shareEntityWithUsers(custosId, request);
                                logger.info("Entity   " + finalEntity.getId() + " shared with user " + userId + " Perm type " + type.getId() + " Cascading False");


                            });

                            List<UserGroup> groups = (new UserGroupRepository()).
                                    getAccessibleGroupsInternal(gatewayId, ent.entityId, permType.permissionTypeId, SharingType.DIRECT_CASCADING);
                            ...
                            groups.forEach(group -> {
                                ...
                                SharingRequest request = SharingRequest.newBuilder()
                                        .setEntity(finalEntity)
                                        .setPermissionType(type)
                                        .addAllOwnerId(Arrays.asList(group.groupId))
                                        .setCascade(true).build();

                                sharingManagementClient.shareEntityWithGroups(custosId, request);
                                logger.info("Entity   " + finalEntity.getId() + " shared with group " + group.groupId + " Perm type " + type.getId() + " Cascading True");


                            });

                            List<UserGroup> NonDirectgroups = (new UserGroupRepository()).
                                    getAccessibleGroupsInternal(gatewayId, ent.entityId, permType.permissionTypeId, SharingType.DIRECT_NON_CASCADING);
                            NonDirectgroups.forEach(group -> {
                                ...
                                SharingRequest request = SharingRequest.newBuilder()
                                        .setEntity(finalEntity)
                                        .setPermissionType(type)
                                        .addAllOwnerId(Arrays.asList(group.getGroupId()))
                                        .setCascade(false).build();


                                sharingManagementClient.shareEntityWithGroups(custosId, request);
                                logger.info("Entity   " + finalEntity.getId() + " shared with group " + group.getGroupId() + " Perm type " + type.getId() + " Cascading False");
                        ...
                        logger.error("Error occured while creating sharings ", e);
                    }

                });
                createCustosSharings(gatewayId, custosId, ent.entityId);
            });
        } catch (Exception ex) {
            logger.error("Error occured while creating sharings ", ex);
        }
        return true;
    }
