...
public class ResourceJobManagerGenerator {
    private static final Logger logger = LoggerFactory.getLogger(ResourceJobManagerGenerator.class);

    private static SQLData createSQLData() {
        SQLData data = new SQLData();
        data.setTableName("RESOURCE_JOB_MANAGER");
        data.getFieldData()
                .put("RESOURCE_JOB_MANAGER_ID", Arrays.asList(new String[] {"VARCHAR", "(255)", "NOT", "NULL"}));
        data.getFieldData()
                .put("RESOURCE_JOB_MANAGER_TYPE", Arrays.asList(new String[] {"VARCHAR", "(255)", "NOT", "NULL"}));
        data.getFieldData().put("PUSH_MONITORING_ENDPOINT", Arrays.asList(new String[] {
            "VARCHAR", "(255)",
        }));
        data.getFieldData().put("JOB_MANAGER_BIN_PATH", Arrays.asList(new String[] {
            "VARCHAR", "(255)",
        }));
