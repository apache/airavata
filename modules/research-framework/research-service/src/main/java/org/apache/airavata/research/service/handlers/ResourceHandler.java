...
public class ResourceHandler {
    ...
    private Page<Resource> getAllPublicResources(int pageNumber, int pageSize, List<Class<? extends Resource>> typeList, String[] tag, String nameSearch) {
    ...
    private Page<Resource> getAllResourcesUserSignedIn(int pageNumber, int pageSize, List<Class<? extends Resource>> typeList, String[] tag, String nameSearch, String userId) {
        Pageable pageable = PageRequest.of(pageNumber, pageSize);

        if (tag == null || tag.length == 0) {
            return resourceRepository.findAllByTypesForUser(
                    typeList, nameSearch.toLowerCase(), userId, pageable);
        }
