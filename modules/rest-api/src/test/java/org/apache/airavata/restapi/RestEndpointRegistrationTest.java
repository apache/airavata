...
public class RestEndpointRegistrationTest {
            ...
            try {
                ...
                if (mapping != null) {
                    ...
                    if (!pathFound && paths.length > 0) {
                        wrongPaths.add(clazz.getSimpleName() + " expected: "
                                + expectedBasePath + " actual: "
        ...
        for (String className : EXPECTED_CONTROLLERS.keySet()) {
            try {
                ...
                if (minimumExpected != null && endpointCount < minimumExpected) {
                    insufficientEndpoints.add(simpleClassName + " has " + endpointCount
                            + " endpoints, expected at least " + minimumExpected);
                }
            } catch (ClassNotFoundException e) {
                // Handled in other test
            }
    ...
    private String getEndpointInfo(Method method, String basePath) {
        ...
        if (getMapping != null) {
            String path = getMapping.value().length > 0
                    ? getMapping.value()[0]
        ...
        if (postMapping != null) {
            String path = postMapping.value().length > 0
                    ? postMapping.value()[0]
        ...
        if (putMapping != null) {
            String path = putMapping.value().length > 0
                    ? putMapping.value()[0]
        ...
        if (deleteMapping != null) {
            String path = deleteMapping.value().length > 0
