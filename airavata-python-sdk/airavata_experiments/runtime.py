...
class Remote(Runtime):
  ...
  def status(self, task: Task) -> tuple[str, States]:
    ...
    from .airavata import AiravataOperator
    from airavata_sdk.generated.org.apache.airavata.model.status.status_pb2 import JobState, ExperimentState
    av = AiravataOperator(AuthContext.get_access_token())
    # prioritize job state, fallback to experiment state
    job_id, job_state = av.get_task_status(task.ref)
    if job_state in [JobState.JOB_STATE_UNKNOWN, JobState.NON_CRITICAL_FAIL]:
      return job_id, ExperimentState.Name(av.get_experiment_status(task.ref))
    else:
      return job_id, JobState.Name(job_state)
...
def find_runtimes(
  ...
  grp_names = [str(x.group_resource_profile_name) for x in grps]
  if group is not None:
    assert group in grp_names, f"Group {group} was not found. Available groups: {repr(grp_names)}"
    groups = [g for g in grps if str(g.group_resource_profile_name) == group]
  else:
    groups = grps
  runtimes = []
  for g in groups:
    matched_runtimes = []
    assert g.group_resource_profile_name is not None, f"Group {g} has no name"
    r: Runtime
    for r in av.get_available_runtimes(group=g.group_resource_profile_name):
