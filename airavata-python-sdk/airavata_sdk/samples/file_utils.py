...
def upload_files(api_server_client, credential_store_client, token, gateway_id, storage_id, storage_host,
    ...
    gateway_storage_preference = api_server_client.get_gateway_storage_preference(gateway_id, storage_id)

    credential = credential_store_client.get_ssh_credential(
        gateway_storage_preference["resourceSpecificCredentialStoreToken"],
        gateway_id)
    ...
    file_handler = FileHandler(storage_host, 22, gateway_storage_preference["loginUserName"], credential["passphrase"],
                               private_key)
    remotePath = gateway_storage_preference["fileSystemRootLocation"] + "/" + username + "/" + project_name + "/" + experiment_id + "/"
