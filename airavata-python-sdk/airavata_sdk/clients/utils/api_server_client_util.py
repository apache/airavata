...
class APIServerClientUtil(object):
    ...
    def __init__(self, gateway_id: str, username: str, password: Optional[str], access_token: Optional[str] = None):
        ...
        if access_token:
            token = self.authenticator.get_airavata_authz_token(
            ...
            token = self.authenticator.get_token_and_user_info_password_flow(
                gateway_id=gateway_id,
                username=username,
                password=password,
            )
        ...
        self.api_server_client = APIServerClient(access_token=token)
    ...
    def get_group_resource_profile_id(self, group_resource_profile_name: str):
        response = self.api_server_client.get_group_resource_list(self.gateway_id)
        for x in response:
    ...
    def get_queue_names(self, resource_host_id: str):
        ...
        batchqueues = resource.get("batchQueues", [])
        return [queue["queueName"] for queue in batchqueues]
